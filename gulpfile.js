...
function installCrossDeps(done) {
  ...
  const badPackages = fs.readdirSync('./node_modules/@fparchive/', { withFileTypes: true }).filter(m => m.isDirectory() && m.name !== 'flashpoint-archive' && m.name.startsWith('flashpoint-archive') && ('@fparchive/' + m.name) !== packageName);
