...
# The standalone Ruffle binary is not shipped in the fp archive; the core-ruffle
# extension downloads it at runtime. When HEADLESS=1, fetch the latest Linux
# build here so quick-sharun can bundle it, matching a machine that has run once.
RUFFLE_DIR="$HOME/Games/Flashpoint/Data/Ruffle/standalone/latest"
if [ "$HEADLESS" = 1 ] && [ ! -x "$RUFFLE_DIR/ruffle" ]; then
	echo "Standalone Ruffle missing; downloading latest Linux build..."
	mkdir -p "$RUFFLE_DIR"
	RUFFLE_URL=$(wget -qO- "https://api.github.com/repos/ruffle-rs/ruffle/releases?per_page=1" | grep -oE 'https://[^"]*linux-x86_64\.tar\.gz' | head -n1)
	wget --retry-connrefused --tries=30 -O /tmp/ruffle.tar.gz "$RUFFLE_URL"
	tar -xzf /tmp/ruffle.tar.gz -C "$RUFFLE_DIR"
	chmod +x "$RUFFLE_DIR/ruffle"
